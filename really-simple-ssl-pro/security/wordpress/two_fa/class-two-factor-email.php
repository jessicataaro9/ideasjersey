...
class Rsssl_Two_Factor_Email extends Rsssl_Two_Factor_Provider {
	...
	public function get_label(): string {
		return _x( 'Email', 'Provider Label', 'really-simple-ssl-pro' );
	}
	...
	public function generate_and_email_token( WP_User $user ): void {
		...
		$subject = wp_strip_all_tags( sprintf( __( 'Your login confirmation code for %s', 'really-simple-ssl-pro' ), wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ) ) );
		/* translators: %s: token */
		...
		$token_html = sprintf(
			'
<table cellspacing="0" cellpadding="0" border="0" width="100%%" style="margin-top: 25px;background-color:white; box-shadow: 1px 3px 0 1px rgba(211, 211, 211, 0.3);">
    <tr>
        <td style="padding-left: 20px; height:60px; vertical-align: middle; font-size: 18px;font-weight:700;">%s</td>
    </tr>
</table>',
			$token_cleaned
		);
		...
		$message = sprintf(
			__( "Below you will find your login code for %1\$s. It's valid for 15 minutes. %2\$s", 'really-simple-ssl-pro' ),
			site_url(),
			$token_html
		);
	...
	public function authentication_page( $user ) {
		...
		?>
		<p class="two-factor-prompt"><?php esc_html_e( 'A verification code has been sent to the email address associated with your account.', 'really-simple-ssl-pro' ); ?></p>
		<p>
			<label for="rsssl-authcode"><?php esc_html_e( 'Verification Code:', 'really-simple-ssl-pro' ); ?></label>
			...
			<?php submit_button( __( 'Log In', 'really-simple-ssl-pro' ) ); ?>
		</p>
		<p class="rsssl-two-factor-email-resend">
			<input type="submit" class="button" name="<?php echo esc_attr( self::RSSSL_INPUT_NAME_RESEND_CODE ); ?>" value="<?php esc_attr_e( 'Resend Code', 'really-simple-ssl-pro' ); ?>" />
		</p>
		...
		foreach ( $user->roles as $role ) {
			...
			if ( 'open' === $provider && in_array( $role, rsssl_get_option( 'two_fa_optional_roles' ), true ) ) {
				$nonce           = wp_create_nonce( 'skip_two_fa_nonce' );
				$skip_two_fa_url = add_query_arg(
					[
						'rsssl_skip_two_fa' => $user->ID,
						'nonce'             => $nonce,
					],
					admin_url()
				);
				...
				<a class="rsssl-skip-link" href="<?php echo esc_url( $skip_two_fa_url ); ?>" style="display: flex; justify-content: center; margin: 15px 20px 0 0;">
					<?php _e( "Don't use Two-Step verification", 'really-simple-ssl-pro' ); ?>
				</a>
				<?php
		...
		<div>
			...
			echo esc_html(
				...
				/* translators: %s: email address */
					__( 'Authentication codes will be sent to %s.', 'really-simple-ssl-pro' ),
					$email
				)
			);
			?>
