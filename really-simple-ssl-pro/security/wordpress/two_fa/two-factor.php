...
class Rsssl_Two_Factor {
	...
	public static function add_hooks( $compat ): void {
		...
		if ( isset( $_GET['rsssl_skip_two_fa'] ) ) {
	...
	public static function maybe_skip_auth() {
		...
		if ( isset( $_GET['rsssl_skip_two_fa'], $_GET['nonce'] ) && wp_verify_nonce( $_GET['nonce'], 'skip_two_fa_nonce' ) ) {

			$user_id = (int) $_GET['rsssl_skip_two_fa'];
	...
	public static function rsssl_filter_authenticate( $user ) {
		if ( $user instanceof WP_User && self::is_api_request() && self::is_user_using_two_factor( $user->ID ) && ! self::is_user_api_login_enabled( $user->ID ) ) {
			return new WP_Error(
				...
				__( 'Error: API login for user disabled.', 'really-simple-ssl-pro' )
			);
		}
	...
	public static function show_two_factor_login( $user ) {
		...
		if ( ! $login_nonce ) {
			wp_die( esc_html__( 'Failed to create a login nonce.', 'really-simple-ssl-pro' ) );
		}
	...
	public static function rsssl_maybe_show_reset_password_notice( $errors ) {
		...
		$errors->add(
			...
			/* translators: %s: URL to reset password */
				__( 'Your password was reset because of too many failed Two Factor attempts. You will need to <a href="%s">create a new password</a> to regain access. Please check your email for more information.', 'really-simple-ssl-pro' ),
	...
	public static function login_html( $user, $login_nonce, $redirect_to, $error_msg = '', $provider = null ) {
		...
		<form name="rsssl_validate_2fa_form" id="loginform" action="<?php echo esc_url( self::login_url( array( 'action' => 'validate_2fa' ), 'login_post' ) ); ?>" method="post" autocomplete="off">
			<input type="hidden" name="provider"      id="provider"      value="<?php echo esc_attr( $provider_class ); ?>" />
			<input type="hidden" name="rsssl-wp-auth-id"    id="rsssl-wp-auth-id"    value="<?php echo esc_attr( $user->ID ); ?>" />
			<input type="hidden" name="rsssl-wp-auth-nonce" id="rsssl-wp-auth-nonce" value="<?php echo esc_attr( $login_nonce ); ?>" />
					...
					<a href="<?php echo esc_url( $login_url ); ?>">
						...
						echo esc_html(
							...
							// translators: %s: Two-factor method name.
								__( 'Or, use your backup method: %s &rarr;', 'really-simple-ssl-pro' ),
			...
			<div class="backup-methods-wrap">
				<p class="backup-methods">
					<a href="javascript:;" onclick="document.querySelector('ul.backup-methods').style.display = 'block';">
						<?php esc_html_e( 'Or, use a backup method…', 'really-simple-ssl-pro' ); ?>
					</a>
				</p>
	...
	public static function rsssl_login_form_validate_2fa() {
		...
		if ( $provider ) {
			...
			if ( isset( $providers[ $provider ] ) ) {
				...
				wp_die( esc_html__( 'Cheatin&#8217; uh?', 'really-simple-ssl-pro' ), 403 );
			}
		} else {
			$provider = self::get_primary_provider_for_user( $user->ID );
		}
		...
		if ( $provider->user_token_has_expired( $user->ID ) ) {
			self::login_html( $user, '', '', esc_html__( 'ERROR: Your verification code expired, click “Resend Code” to receive a new verification code.', 'really-simple-ssl-pro' ), $provider );
			exit;
		}
		...
		if ( true === self::is_user_rate_limited( $user ) ) {
			...
			$error = new WP_Error(
				...
				sprintf(
					...
					__( 'ERROR: Too many invalid verification codes, you can try again in %s. This limit protects your account against automated attacks.', 'really-simple-ssl-pro' ),
		...
		if ( true !== $provider->validate_authentication( $user ) ) {
			do_action( 'rsssl_wp_login_failed', $user->user_login, new WP_Error( 'rsssl_two_factor_invalid', __( 'ERROR: Invalid verification code.', 'really-simple-ssl-pro' ) ) );//phpcs:ignore
			...
			if ( $provider->user_token_has_expired( $user->ID ) ) {
				self::login_html( $user, $login_nonce['rsssl_key'], $_REQUEST['redirect_to'], esc_html__( 'ERROR: Your verification code expired, click “Resend Code” to receive a new verification code.', 'really-simple-ssl-pro' ), $provider );
				exit;
			} else {
				self::login_html( $user, $login_nonce['rsssl_key'], $_REQUEST['redirect_to'], esc_html__( 'ERROR: Invalid verification code.', 'really-simple-ssl-pro' ), $provider );
				exit;
			}
		}
		...
		if ( $interim_login ) {
			...
			$message       = '<p class="message">' . __( 'You have logged in successfully.', 'really-simple-ssl-pro' ) . '</p>';
...
This is an automated notification. If you would like to speak to a site administrator, please contact them directly.',
		...
		$mailer->subject = __( 'Your password was compromised and has been reset', 'really-simple-ssl-pro' );
		$mailer->branded = false;
									/* translators: %s: site url */
		$mailer->sent_by_text      = sprintf( __( 'Notification by %s', 'really-simple-ssl-pro' ), site_url() );
		...
		$mailer->title             = __( 'Hi', 'really-simple-ssl-pro' ) . ' ' . $user->display_name . ',';
	...
	public static function notify_admin_user_password_reset( $user ) {
		$subject = sprintf(
			...
			__( 'Compromised password for %s has been reset', 'really-simple-ssl-pro' ),
			esc_html( $user->user_login )
		);
...
Thank you',
		...
		$mailer->template_filename = apply_filters( 'rsssl_email_template', rsssl_path . '/mailer/templates/email-unbranded.html' );
		$mailer->to                = $user->user_email;
		$mailer->title             = __( 'Compromised password reset', 'really-simple-ssl-pro' );
		$mailer->message           = $message;
		$mailer->send_mail();
	}
	...
	public static function show_password_reset_error() {
		$error = new WP_Error(
			...
			sprintf(
				...
				__( 'There have been too many failed two-factor authentication attempts, which often indicates that the password has been compromised. The password has been reset in order to protect the account.', 'really-simple-ssl-pro' ),
				__( 'If you are the owner of this account, please check your email for instructions on regaining access.', 'really-simple-ssl-pro' )
			)
		);

		login_header( __( 'Password Reset', 'really-simple-ssl-pro' ), '', $error );
		login_footer();
	}
