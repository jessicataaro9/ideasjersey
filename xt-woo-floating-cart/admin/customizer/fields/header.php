...
if ( $this->core->access_manager()->can_use_premium_code__premium_only() ) {
    ...
    $fields[] = array(
        'id'          => 'cart_shipping_bar_amount_display',
        'section'     => 'header',
        'label'       => esc_html__( 'Amount Left Display', 'woo-floating-cart' ),
        'description' => sprintf(esc_html('By default, the amount left will be displayed Including or Excluding taxes based on the native WooCommerce option: %s"Display prices during cart and checkout."%s You can however choose to override it here. Note that, the minimum order amount set to get free shipping is always calculated before tax.', 'xt-woo-floating-cart'), '<strong>', '</strong>'),
        'type'        => 'radio-buttonset',
        'choices'     => array(
            'default' => esc_attr__( 'Based on WooCommerce Settings', 'woo-floating-cart' ),
            'exc_tax' => esc_attr__( 'Excluding Tax', 'woo-floating-cart' ),
            'inc_tax' => esc_attr__( 'Including Tax', 'woo-floating-cart' )
        ),
        'transport'   => 'postMessage',
        'partial_refresh'    => [
            'cart_shipping_bar_amount_display' => [
