...
class XT_Woo_Floating_Cart_Public_Pro extends XT_Woo_Floating_Cart_Public
{
    ...
    public function __construct($core)
    {
        ...
        if (!is_admin()) {
            add_filter('wc_get_template', array($this, 'wc_get_template_cart_shipping'), 10, 3);
        }
    ...
    public function wc_get_template_cart_shipping($located, $template_name, $args = array())
    {
        if (
            strpos($located, 'cart/cart-shipping.php') !== false &&
            ($this->enabled() && $this->totals_enabled()) &&
            did_action('xt_woofc_before_totals')
        ) {

            $located = $this->core->get_template('parts/cart/shipping', $args, false, true);
        }
    ...
    public function render_shipping_bar($partial = false){
        ...
        $amountLeft = wc_price( $data['amount_left'] );

        if ( $this->shipping_bar_display_including_tax() ) {
            $amountLeft .=  ' <small class="tax_label">' .WC()->countries->inc_tax_or_vat(). '</small>';
        }

        $args = array();
        $args['partial'] = $partial;
        $args['shipping_text'] = $data['is_free'] ? $free_text : str_replace( '%s', '<strong>'.$amountLeft.'</strong>', $remaining_text);
    ...
    public function shipping_bar_display_including_tax() {

        if(!wc_tax_enabled()) {
            return false;
        }

        $amount_display = $this->core->customizer()->get_option('cart_shipping_bar_amount_display', 'default');

        if($amount_display === 'default') {
            return WC()->cart->display_prices_including_tax();
        }else{
            return $amount_display === 'inc_tax';
        }
    }
            ...
            foreach ( $shipping_methods as $obj ) {
                ...
                if( $obj instanceof WC_Shipping_Free_Shipping && ( $obj->requires === 'min_amount' || $obj->requires === 'either' ) ){
                    ...
                    if( $obj->ignore_discounts === "no" && !empty( WC()->cart->get_coupon_discount_totals() ) ){
                        foreach ( WC()->cart->get_coupon_discount_totals() as $coupon_code => $discount_amount ) {

                            if ( $this->shipping_bar_display_including_tax() ) {
                                $discount_amount += WC()->cart->get_coupon_discount_tax_amount($coupon_code);
                            }
